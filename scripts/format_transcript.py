...
# -*- coding: utf-8 -*-
"""
ASR Transcript Deep Technical & Verbatim Paragraph Formatter
Preserves 100% of the verbatim spoken narrative while structuring the text
into beautifully readable paragraphs, speaker cues, and clear thematic subheadings.
"""
...
def deep_correct_91(raw: str) -> str:
    ...
    replacements = [
        ...
        ("主要是 F 在這個上面的一些一些肉挖掘跟利用", "主要是 Mali GPU 在這上面的一些漏洞挖掘跟利用"),
        ...
        ("有效限制一些來入不明的 APP, 應該說他規範了每個 APP 它可以做什麼樣的事情。那一個對於一個來顧名的 AP 在執行的時候，他 SN 給的權限是非常非常的少", "有效限制來路不明的 App，規範了每個 App 可以做什麼事。對於來路不明的 App 執行時，SELinux 給的權限非常非常少"),
...
def deep_correct_92(raw: str) -> str:
    ...
    replacements = [
        ...
        ("磨改了 B, 把改名叫 XCB", "魔改了 adbd，把它改名叫做 xcbd"),
...
def deep_correct_94(raw: str) -> str:
    ...
    replacements = [
        ...
        ("我拿客去 C, 然後他把它罵爆。所以這波給他玩，然後客把它全部夠好了", "拿去給 Claude 生成被我罵爆之後，Claude 把它全部寫好修好了"),
        ("現在 GBT 是我大學", "現在 GPT 是我乾爹"),
        ("存 ing 是什麼意思？就這我加電，然後大概就是這樣子。那除了改變數以外，我其實一行 C 都沒有寫", "Prompting 是什麼意思？就是加提示詞。除了改變數名稱以外，我其實一行 Code 都沒寫"),
        ("我畫風一轉，這個整個專都是 coding 出來的", "我畫風一轉，這個整個專案都是 Prompt Engineering 生成出來的"),
...
def build_structured_paragraphs(raw_text: str, stem: str) -> str:
    """
    Format narrative into clean, comfortable paragraphs with thematic subheadings,
    preserving 100% of the verbatim text.
    """
    cleaned = clean_spaces_and_punct(raw_text)

    if stem == "91":
        corrected = deep_correct_91(cleaned)
        # Add narrative subheadings at key transition phrases without losing any text
        corrected = re.sub(r"(我們切換好。大家好，我是 PK)", r"\n\n## 👤 講者介紹與 Android 核心研究契機\n\n\1", corrected)
        corrected = re.sub(r"(今天的 L 大概是長這樣)", r"\n\n## 📋 今日議程大綱\n\n\1", corrected)
        corrected = re.sub(r"(因為我之前是做一般系統研究)", r"\n\n## 🎯 研究目標：Google Pixel 8A 與攻擊情境設定\n\n\1", corrected)
        corrected = re.sub(r"(指定級不一樣。跑在 x86_64)", r"\n\n## 🛡️ Android 核心防禦機制與第三方 GPU 驅動攻擊面\n\n\1", corrected)
        corrected = re.sub(r"(基本上絕大多數漏洞都是第三方的 driver)", r"\n\n## 🔬 建立除錯環境：CONFIG_MALI_BIFROST_NO_MALI 與 kprobes\n\n\1", corrected)
        corrected = re.sub(r"(Mali GPU 驅動一些比較重要的架構背景知識)", r"\n\n## ⚙️ Mali GPU 驅動架構：kbase 與 Queue 機制\n\n\1", corrected)
        corrected = re.sub(r"(CVE-2025-8045)", r"\n\n## 💥 漏洞一：CVE-2025-8045 Double Free 成因\n\n\1", corrected)
        corrected = re.sub(r"(CVE-2025-6349。那這個漏洞)", r"\n\n## 💥 漏洞二：CVE-2025-6349 Queue UAF 挖到 0-Day 的過程\n\n\1", corrected)
        corrected = re.sub(r"(將難用的 Primitive 轉換成好用的 Primitive)", r"\n\n## 🧩 Exploit 構造：Primitive 轉換與 Pipe Page 佔位\n\n\1", corrected)
        corrected = re.sub(r"(修改函式指標後立刻觸發 Kernel Panic)", r"\n\n## 🛡️ 繞過 Clang Forward-Edge CFI\n\n\1", corrected)
        corrected = re.sub(r"(ARM64 Page Table 中第一層目錄)", r"\n\n## 👑 覆寫 Page Table Entry (PTE) 達成 Full Root\n\n\1", corrected)
        corrected = re.sub(r"(那這是我 Demo 的影片)", r"\n\n## 🎬 實機 Demo 影片展示\n\n\1", corrected)
        corrected = re.sub(r"(那最後講一下心得好了)", r"\n\n## 💡 資安研究心得與問答\n\n\1", corrected)

    elif stem == "92":
        corrected = deep_correct_92(cleaned)
        corrected = re.sub(r"(開個非標準 ADB 埠幹嘛)", r"\n\n## 🔍 探索 POS 刷卡機：發現魔改 ADB 服務 xcbd\n\n\1", corrected)
        corrected = re.sub(r"(AI 就說「我放棄了，要換目標了」)", r"\n\n## 🤖 AI 逆向挖洞困境：太早放棄的 LLM\n\n\1", corrected)
        corrected = re.sub(r"(做出了一個「Never Giving Up」自動化框架)", r"\n\n## 🦞 OpenClaw + Claude：24 小時微壓榨挖出 3 個 0-Day\n\n\1", corrected)
        corrected = re.sub(r"(SCREEN_KEY 與 FRAMEBUFFER API)", r"\n\n## 💥 0-Day #1: 免驗證按鍵側錄與螢幕截圖\n\n\1", corrected)
        corrected = re.sub(r"(換一個 Target，去看安裝流程的 Binary)", r"\n\n## 💥 0-Day #2: 繞過 RSA-2048 簽章驗證\n\n\1", corrected)
        corrected = re.sub(r"(Zip-Slip 與 Symlink 任意檔案覆寫漏洞)", r"\n\n## 💥 0-Day #3: Zip-Slip 與 Symlink 覆寫 Root RCE\n\n\1", corrected)
        corrected = re.sub(r"(拿來玩《俄羅斯方塊》)", r"\n\n## 🎮 刷卡機改裝大作戰：俄羅斯方塊、Bad Apple 與 Rickroll\n\n\1", corrected)
        corrected = re.sub(r"(廠商說這款設備已經推出 8 年了)", r"\n\n## 📢 漏洞通報與負責任揭露的現實困境\n\n\1", corrected)
        corrected = re.sub(r"(好奇心與想像力是 AI 時代人類無法被超越的超能力)", r"\n\n## 💭 總結：想像力是你的超能力\n\n\1", corrected)
        corrected = re.sub(r"(現場提問|如何繞過 AI 的道德審查限制|使用 Apple Pay / Google Pay 是否較安全)", r"\n\n## ❓ 現場 Q&A 問答環節\n\n\1", corrected, count=1)

    elif stem == "93":
        corrected = deep_correct_93(cleaned)
        corrected = re.sub(r"(各位貴賓大家好。這場議程主要是要分享)", r"\n\n## 🎯 議程引言：黑吃黑——瞄準資安研究員與紅隊的供應鏈攻擊\n\n\1", corrected)
        corrected = re.sub(r"(我是 Sam，在我旁邊的是 Jason)", r"\n\n## 👤 講者介紹與背景\n\n\1", corrected)
        corrected = re.sub(r"(今天 Outline 我們分為七個章節)", r"\n\n## 📋 議程大綱與研究背景\n\n\1", corrected)
        corrected = re.sub(r"(微軟 WSUS 重大漏洞)", r"\n\n## 🎣 攻擊手法拆解：GitHub 假 PoC 與惡意 PyPI 相依鏈\n\n\1", corrected)
        corrected = re.sub(r"(PyPI 上 74 萬多個套件)", r"\n\n## ⚡ 高效過濾分析：利用 ZIP 檔尾目錄檢測 74 萬套件\n\n\1", corrected)
        corrected = re.sub(r"(Execution Context Keying：動態讀取最外層執行的腳本檔名)", r"\n\n## 🔐 高級反分析技術：Execution Context Keying\n\n\1", corrected)
        corrected = re.sub(r"(利用合法雲端服務|寫入 sitecustomize.py)", r"\n\n## ☁️ C2 通訊與 Persistence 常駐維持機制\n\n\1", corrected, count=1)
        corrected = re.sub(r"(工作作息高度集中於 UTC\+8 時區)", r"\n\n## 🕵️ APT 威脅情資分析：作息時區、春節停工與受害者特徵\n\n\1", corrected)
        corrected = re.sub(r"(總結這場演講)", r"\n\n## 🛡️ 防禦建議與總結\n\n\1", corrected)

    elif stem == "94":
        corrected = deep_correct_94(cleaned)
        corrected = re.sub(r"(來看看不一樣的日常，掌聲歡迎 Henry！)", r"\n\n## ⚡ 閃電秀 1：DEFCON Goon 現場維安工作人員招募（Henry）\n\n\1", corrected)
        corrected = re.sub(r"(克雷帶來的主題：讓 HITCON 成為你的知識庫)", r"\n\n## ⚡ 閃電秀 2：讓 HITCON 成為你的知識庫——HITCON KB 2.0 (HITCON Wiki)（克雷）\n\n\1", corrected)
        corrected = re.sub(r"(《Mina 誕生於神話，凌駕於神話之上》Minerva 是羅馬神話智慧女神)", r"\n\n## ⚡ 閃電秀 3：極致 Cyberpunk 中二 C2 框架 Mina（Ray）\n\n\1", corrected)
        corrected = re.sub(r"(HITCON 2026 的活動組|活動組新創開發 2026)", r"\n\n## ⚡ 閃電秀 4：HITCON 2026 活動組幕後除障血淚史（活動組）\n\n\1", corrected, count=1)
        corrected = re.sub(r"(第五位講者阿斯卡，題目叫做 PowerShell)", r"\n\n## ⚡ 閃電秀 5：PowerShell 隱蔽執行與 TypeData 屬性覆寫奇技淫巧（阿斯卡）\n\n\1", corrected)
        corrected = re.sub(r"(《來自超自然的震動：Bug Hunting》|超自然的震動：Bug Hunting)", r"\n\n## ⚡ 閃電秀 6：來自超自然的震動——智慧成人連網玩具漏洞挖掘（S & 艾子）\n\n\1", corrected, count=1)

    else:
        corrected = cleaned

    # Split into clean paragraphs (1-3 sentences per paragraph for natural narrative flow)
    ...
    for line in lines:
        ...
        for piece in raw_pieces:
            ...
            # Break into a clean, readable narrative paragraph around 120-180 characters or 2-3 sentences
            if len(current_p) >= 2 and current_len >= 120:
                p_text = "".join(current_p).strip()
                if p_text:
                    final_blocks.append(p_text + "\n")
                current_p = []
                current_len = 0
    ...
    title_map = {
        ...
        "93": "黑吃黑：瞄準資安研究員與紅隊的供應鏈攻擊 (Jason & Sam)",
        "94": "HITCON 2026 閃電秀全集（6 場短講合輯）",
    }
    file_title = title_map.get(stem, f"{stem} 演講全文逐字稿【100% 完整原話深度分段校對版】")
...
def format_file(input_file: Path, output_file: Path):
    ...
    result_md = build_structured_paragraphs(raw_text, input_file.stem)

    with open(output_file, "w", encoding="utf-8") as f:
        f.write(result_md)

    print(f"✔ 100% 原話自然分段與校對完成：{input_file.name} -> {output_file.name} ({len(result_md)} 字元)")


def main():
    parser = argparse.ArgumentParser(description="ASR 逐字稿 100% 原話無損深度分段與校對工具")
    parser.add_argument("-i", "--input", default=".", help="輸入檔案或資料夾路徑")
    parser.add_argument("-o", "--output", help="輸出 Markdown 檔案或資料夾路徑")

    args = parser.parse_args()
    in_dir = Path(args.input)

    files = ["91-Pixel8A-GPU漏洞挖掘-raw.txt", "92-POS-ADB-0Day-AI輔助-raw.txt", "93-供應鏈攻擊-黑吃黑-raw.txt", "94-閃電秀6場合輯-raw.txt"]
    for fname in files:
        fpath = in_dir / fname
        if fpath.exists():
            out_name = f"{fpath.stem}_proofread.md"
            out_path = in_dir / out_name
            format_file(fpath, out_path)
